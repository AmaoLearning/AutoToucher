...
**典型用例**：每天定时自动完成钉钉打卡（点击图标 → 工作台 → 打卡 → 上/下班打卡 → 返回两次）
...
- 每个任务包含一个**有序点击列表**，支持任意条目数（推荐 4 步完成钉钉打卡）。
- 每条点击项包含：
  - **坐标**：屏幕像素坐标 `(x, y)`（以左上角为原点）；
  - **操作类型**：`TAP`（单击）| `BACK`（返回）| `HOME`（回桌面）| `CLOSE_ALL`（清除所有后台）。
- 相邻两次点击之间等待**随机间隔**，范围由任务级别统一配置（默认 `[4, 8]` 秒，可按条目覆盖）。

> **典型钉钉打卡序列示例（4 点击 + 2 返回）**：
>
> | 步骤 | 操作 | 坐标 / 动作 | 说明 |
> |------|------|-------------|------|
> | 1 | TAP | (x1, y1) | 点击桌面钉钉图标 |
> | 2 | TAP | (x2, y2) | 点击「工作台」 |
> | 3 | TAP | (x3, y3) | 点击「打卡」 |
> | 4 | TAP | (x4, y4) | 点击「上班打卡」或「下班打卡」 |
> | 5 | BACK | — | 返回钉钉主界面（等待打卡完成动画） |
> | 6 | BACK | — | 返回桌面 |
